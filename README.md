...
## Systemd setup (overnight mining)
...
The miner runs automatically from **23:00 to 07:00**.
...
```
*/15 23,0-6 * * * /path/to/cudalotto/btc-miner-monitor.sh
```

The monitor script reads `ALERT_EMAIL` and `LOGFILE` from `.env`. It:
- Checks if the miner is running during night hours
